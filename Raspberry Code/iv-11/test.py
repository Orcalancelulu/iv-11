...
async def find_temp_sensor():
    ...
    async with aioble.scan(100000, interval_us=30000, window_us=30000, active=True) as scanner:
        async for result in scanner:
            # See if it matches our name and the environmental sensing service.
            if (result.name() not in nameFilter):
                print(result.name())
                if (result.name() not in idList):
                    idList.append(result.name())
                    #print(result.name())
